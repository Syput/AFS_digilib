...
function tampil(){
     ...
     table = $('#databuku').DataTable({
        ...
        "ajax": {
            "url": '<?php echo base_url()."index.php/anggota/buku/ambilData"?>',
            "type": "POST"
        },
    ...
    function read_pustaka(judul,kode){
       // alert(judul);
        var x = window.open('<?php echo base_url()."index.php/anggota/buku/baca/";?>' + judul + '/' + kode,'_blank');
        x.focus();
	  ...
	  swal({
            ...
            .then((willDelete) => {
              if (willDelete) {
                  $.ajax({
                    type:'POST',
                    url: '<?php echo base_url()."index.php/anggota/buku/tambah_sitasi"?>',
                    data: {kode_s:kode},
                    dataType:'json',
                    success:function(){
